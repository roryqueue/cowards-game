...
const proofJsonPath = path.join(
  artifactDir,
  "v1.20-signed-in-reliability-proof.json",
)
const proofMarkdownPath = path.join(
  artifactDir,
  "v1.20-signed-in-reliability-proof.md",
)
...
test.skip(
  process.env.RUN_V1_20_PROOF !== "1",
  "v1.20 signed-in reliability proof requires live web, Go backend, runtime-service, Postgres, and RUN_V1_20_PROOF=1.",
)
...
interface VerifiedExhibition extends ProofExhibition {
  matchup: "js-ts-vs-python" | "python-vs-python"
...
const settleExhibitions = async (
  ...
  summaries: PublicMatchSetSummary[]
  ...
  while (Date.now() - startedAt <= options.maxWaitMs) {
    ...
    if (!summaries.some(isUnsettled)) {
      return { summaries, workerStatuses }
    }
...
const verifyResultAndReplay = async (
  ...
  const resultTiming = await timed(async () => {
    ...
    await expect(
      matchSetEvidence.getByText(/Docker\/container is v1\.20 readiness/i),
    ).toBeVisible()
    await expect(
      matchSetEvidence.getByText(/three bounded cycles/i),
    ).toBeVisible()
    await expect(
      matchSetEvidence.getByText(/execution-path proof is gated/i),
    ).toBeVisible()
    await expect(
      matchSetEvidence.getByText(/Python · non-counted exhibition beta/),
    ).toBeVisible()
...
const writeProofArtifacts = (proof: unknown): void => {
  ...
  const lines = [
    ...
    "## Promotion Decision",
    "",
    `- Python: ${summary.promotionDecision.pythonStatus}.`,
    `- Runtime isolation: ${summary.promotionDecision.runtimeIsolationStatus}.`,
    "",
...
test("signed-in v1.20 reliability proof covers repeated Python exhibitions", async ({
  page,
}) => {
  ...
  const cycles: {
    ...
    revisions: { jsTs: string; python: string[] }
    createMs: number
    workerMs: number
    settleMs: number
    workerStatuses: number[]
  ...
  for (let cycle = 1; cycle <= 3; cycle += 1) {
    const handle = `v120-${suffix}-${cycle}`
    const signupTiming = await timed(async () => {
      const signup = await page.request.post("/api/auth/sign-up", {
        data: {
          username: `v120_${suffix}_${cycle}`,
          handle,
          displayName: `v1.20 Reliability Proof ${cycle}`,
          password: `v120-proof-${suffix}-${cycle}`,
        },
      })
      expect(signup.status(), await signup.text()).toBe(201)
    })
    const [tsRevisionId, pythonScreenId, pythonPressureId] = await Promise.all([
      saveRevision(page, {
        source: workshopBody.templateSource,
        sourceFormat: "typescript",
        label: `v1.20 JS/TS reliability proof revision ${cycle}`,
        notes: "Signed-in v1.20 JS/TS regression proof.",
      }),
      saveRevision(page, {
        source: pythonScreenSource,
        sourceFormat: "python",
        label: `v1.20 Python screen reliability revision ${cycle}`,
        notes: "Signed-in v1.20 non-counted exhibition beta proof.",
      }),
      saveRevision(page, {
        source: pythonPressureSource,
        sourceFormat: "python",
        label: `v1.20 Python pressure reliability revision ${cycle}`,
        notes: "Signed-in v1.20 non-counted exhibition beta proof.",
      }),
    ])
    ...
    const created = await timed(async () => {
      const mixed = await createExhibition(page, [tsRevisionId, pythonScreenId])
      const pythonOnly = await createExhibition(page, [
        pythonScreenId,
        pythonPressureId,
      ])
      return { mixed, pythonOnly }
    })
    const worker = await timed(async () => runWorkerIterations(page, 6))
    const exhibitions = [created.value.mixed, created.value.pythonOnly]
    const settle = await timed(async () =>
      settleExhibitions(page, exhibitions, {
        maxExtraIterations: 12,
        maxWaitMs: 180_000,
      }),
    )
    ...
    cycles.push({
      ...
      revisions: {
        jsTs: tsRevisionId,
        python: [pythonScreenId, pythonPressureId],
      },
  ...
  const proof = {
    schemaVersion: "v1.20-signed-in-reliability-proof",
    ...
    totals: {
      ...
      pythonVsPythonChecked: true,
      mixedJsTsVsPythonChecked: true,
    },
    runtimePath:
      "web frontend -> Go backend -> Strategy Execution Service / Runtime Broker -> selected runtime implementation",
    runtimeServiceAdapterEvidence:
      process.env.COWARDS_RUNTIME_SERVICE_PROOF_ADAPTER ??
      "not supplied by proof runner",
    candidateEvidence: {
      containerReadiness: readJsonArtifact(
        ".planning/artifacts/v1.20-runtime-sandbox-candidate-readiness.container.json",
      ),
      runscFailLoud:
        "runsc remains a strict fail-loud host-runtime preflight unless genuinely available.",
    },
    promotionDecision: {
      pythonStatus: "non-counted exhibition beta only",
      runtimeIsolationStatus:
        "readiness evidence only; no production sandbox certification",
    },
  }
