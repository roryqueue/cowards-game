  ...
  try {
    ...
    if (isGoAccountForksSelected()) {
      ...
      const created = await requireSelectedGoBackendClient(
        "account revisions",
      ).forkStarterStrategy(await getAccountSessionId(), body.starterId)
      const revision =
        (await listAccountReadRevisions()).find(
          (candidate) => candidate.id === created.strategyRevisionId,
        ) ?? null
      if (!revision) {
        throw new Error("Go starter fork did not return a listable revision.")
      }
    ...
    const revision = await competitiveServer.forkStarterStrategy(user, {
