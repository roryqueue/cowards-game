...
This is a deliberately small v1.8 read-only backend boundary. It proves a Go service can serve DTO envelopes from generated TypeScript parity fixtures without taking over orchestration, persistence ownership, or Strategy execution.
...
Go remains read-only in v1.8. TypeScript still owns auth mutation, Strategy submission, Strategy source retrieval, MatchSet creation, Match orchestration, job claiming, migrations, persistence writes, and Strategy execution. The analytics summary endpoint is owner-scoped parity evidence only; the Go spike requires a locally configured bearer-token identity and does not promote a public analytics product route.
