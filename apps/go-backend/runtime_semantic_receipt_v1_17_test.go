...
func TestPhase258RuntimeSemanticReceiptV117GoldenParity(t *testing.T) {
	...
	var wire struct {
		Result struct { SemanticReceipt runtimeSemanticReceiptV117 `json:"semanticReceipt"` } `json:"result"`
	}
	if err := json.Unmarshal(bytes, &wire); err != nil { t.Fatal(err) }
...
func TestPhase258RuntimeSemanticReceiptV117ClosedVersionDispatch(t *testing.T) {
	for _, schema := range []string{"", "runtime-semantic-receipt-v1", "runtime-semantic-receipt-v1.16", "runtime-semantic-receipt-v1.18"} {
		if runtimeSemanticReceiptV117SchemaKnown(schema) { t.Fatalf("unknown schema %q accepted", schema) }
	}
	if !runtimeSemanticReceiptV117SchemaKnown("runtime-semantic-receipt-v1.17") { t.Fatal("v1.17 schema missing") }
}
