...
| Property | Value |
|----------|-------|
| **Frameworks** | Vitest 4, package test scripts, Go `testing`, real Python/Rust/Zig toolchains, Wasmtime, PostgreSQL integration proof |
...
|-------------|--------------------|-----------------|-----------|--------------------------|--------|
| CONF-01 | Immutable corpus manifest and real TS/Python/Rust/Zig executor fixtures | No declaration, fallback, unsupported result, or partial lane can count as execution | integration | `pnpm --filter @cowards/golden test` plus phase certificate runner | ⬜ pending |
...
| CONF-03 | Boundary/generated/mutation/failure lane-by-case matrix | Missing capability, skip, nondeterministic seed, or surviving mutation fails the certificate | adversarial | phase corpus evaluator `--check` | ⬜ pending |
| CONF-04 | Complete identity mutation table | Any bound engine, adapter, runtime, toolchain, ABI, policy, corpus, or artifact change stales evidence | mutation | phase certificate freshness tests | ⬜ pending |
| CONF-05 | Three-run certificate verifier and Phase-256 authority integration fixtures | Only a current complete reviewed artifact hash promotes its lane | integration/DB | certificate evaluator plus persistence authority tests | ⬜ pending |
...
1. executes the complete immutable corpus three times in fresh processes for TypeScript, Python, Rust, and Zig;
2. requires identical full-trace and evidence roots with no skip, unsupported, fallback, or synthetic success;
3. verifies certificate staleness for every bound identity;
4. runs current per-slot Chronicle validation and per-transition reconstruction;
5. verifies frozen v1.4 bytes and interpretation under explicit historical dispatch;
6. proves shared runtime-service/PostgreSQL admission, rollback, and privacy-safe projections; and
7. runs the audit reproduction, Go, contract, boundary, lint, and typecheck suites.

## Manual-Only Verifications

All required phase behaviors must be automated. Human review of a candidate golden semantic diff is an approval boundary, not a substitute for executable proof. Any diff that changes valid v1.4 state, legality, event order, outcome, or Strategy observation stops for explicit compatibility approval.
...
- [ ] No watch-mode, pass-with-no-tests, skipped semantic case, or synthetic lane result.
- [ ] Focused feedback target is under 120 seconds.
- [ ] Final gate requires all four real lanes and immutable historical compatibility.
