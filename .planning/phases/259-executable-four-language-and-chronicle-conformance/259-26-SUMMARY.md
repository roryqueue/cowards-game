...
- Native Rust tests — 6/6 passed locally and 6/6 passed in the exact pinned Rust Linux builder.
- `cargo tree --locked --depth 1` — only `cowards-runtime-supervisor`; zero crate dependencies.
- Focused native/build/controller tests — 8/8 passed.
- `@cowards/runtime-supervisor` package tests — 16/16 passed.
- Package build, typecheck, and lint — passed.
- Exact `--build --check` manifest reproduction — passed.
- Real `--build-linux-container --check` cgroupfs-v2 certification — passed.
