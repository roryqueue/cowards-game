...
Resolution: the replay page no longer reads `mode` or `ownerPlayerId` from URL search params. The server facade now requires `allowOwnerDebug: true` in addition to `mode: "owner"` and `ownerPlayerId`, preserving an internal trusted path for future authenticated server code while keeping direct route requests public. `apps/web/app/matches/server.test.ts` covers both untrusted owner-mode fallback and trusted owner-mode projection.
...
Resolution: Pixi application initialization now runs once for the React-owned canvas, stores the app in a ref, and separates redraw from app lifecycle. Timeline changes redraw the existing app, stage children are destroyed when replaced, and unmount cleanup destroys the Pixi application while preserving the canvas element.
...
Resolution: fixture ID decoding now goes through a guarded helper that returns `null` on malformed input. `apps/web/app/matches/server.test.ts` covers malformed encoded IDs and the encoded fixture ID path.
...
```bash
pnpm --filter @cowards/web test -- app/matches/server.test.ts 'app/matches/[matchId]/replay/replay-board.test.ts'
