# Phase 169: Direct-Export ABI Proof Spike - Context
...
This phase clarifies and implements only the requirements mapped to Phase 169: DEX-01, DEX-02, DEX-03, DEX-04, DEX-05, DEX-06. New runtime promotion, counted non-JS support, production sandbox certification, or ABI migration outside this phase boundary must be deferred unless explicitly required by the mapped requirements.
...
### Approved Defaults
- **D-01:** A minimal artifact-only proof is acceptable.
- **D-02:** Success means evidence about memory ownership, allocation/free, buffer passing, encoding, caps, schema validation, failure behavior, and Rust/Zig parity.
- **D-03:** Fail-loud non-promotion is acceptable and likely if parity or memory ownership is not strong enough.
- **D-04:** Direct-export artifacts must be ineligible for Match execution without a future explicit promotion decision.
- **D-05:** Record compatibility and rollback impact against existing Preview 1 JSON artifacts.
