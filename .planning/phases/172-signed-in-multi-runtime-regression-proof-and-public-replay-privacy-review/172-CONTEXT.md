# Phase 172: Signed-In Multi-Runtime Regression Proof and Public Replay/Privacy Review - Context
...
Prove JS/TS counted support and Python/Rust/Zig beta regressions still work with public-safe result/replay evidence.

This phase clarifies and implements only the requirements mapped to Phase 172: REG-01, REG-02, REG-03, REG-04, REG-05, REG-06, REG-07, REG-08. New runtime promotion, counted non-JS support, production sandbox certification, or ABI migration outside this phase boundary must be deferred unless explicitly required by the mapped requirements.
...
### Approved Defaults
- **D-01:** Reuse v1.23 signed-in proof style but broaden privacy and no-fallback evidence for v1.24.
- **D-02:** Proof must cover JS/TS counted support and Python, Rust, and Zig non-counted exhibition beta support.
- **D-03:** Open result pages and replay pages for proof evidence.
- **D-04:** Replay plausibility remains a gate: full Match start, visible Soldiers and terrain inside declared board bounds, nonblank board.
- **D-05:** Public result/replay output must pass private-data scans and no silent fallback evidence must be recorded.
