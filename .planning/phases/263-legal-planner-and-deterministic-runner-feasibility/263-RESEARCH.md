...
| `src/runtime-bridge.ts` | engine kernel driver/types; runtime-js selected-v1.19 executor/abi-bridge and lean container session execute | Core accepts typed injected bound results; host uses the actual selected container lane, not counted-v1.18 supervisor migration. |
...
2. **Wrong time budget/lane.** The executed262 retry binds DEFAULT_RUNTIME_LIMITS (1000ms timeout,262144 stdout,65536 stderr/source,32768 StrategyMemory,2048 SoldierMemory,1024 objective) and ABI-v1.19 through createSelectedCurrentRuntimeFromRevisionV119 → executeSelectedCurrentStrategyRuntimeAbiV119 → session.adapter.execute/runLegacy. The outer service envelope is v1.18, but its invocation is not the counted-v1.18 supervisor factory or50ms budget. Preserve exact2CPU/256m pinned container controls and120-second cell/60-minute outer bounds. The independent direct-method p99 gate remains strictly below5ms. [VERIFIED: v1.38-lean-closeout-retry-source-review.json binding; runtime.ts:573; lean runner:659,676,704–729; execute-match.ts:1000–1006; executor.ts:249–258; abi-bridge.ts:259]
...
## Open Questions — RESOLVED in checked-plan revision

1. **RESOLVED: Executable benchmark binding.** Plan01 preserves benchmark/hardware labels and policy implementation root, adding subordinate corpus/harness hashes before results. Each method gets100 warmups+1000 measured calls; nearest-rank sample990 must be below5ms. Plan04 specifies trusted method.call observer outside guest lexical scope and a bounded owned-channel timing record bound to invocation/source/input/method/tuple/harness/profile; no transport timing substitution. Plan06 freezes exact identities before sole run.
2. **RESOLVED: Generic runtime bridge.** Plan04 uses private host → createSelectedCurrentRuntimeFromRevisionV119 → executeSelectedCurrentStrategyRuntimeAbiV119 → createLeanContainerMatchSession(...closeout...).adapter.execute. Existing selected executor normalizes results; core pumps MATCH_KERNEL only. New source/request/artifact/profile admission and charged private host observations replace fixture certificate/accounting shortcuts without claiming counted/service signatures. No app import, executeV117 or counted-v1.18 migration; selected-v1.19 roundtrip/profile-mismatch and observer fault tests are required.
3. **RESOLVED: Phase allocation.** Plans01/06 fix24 attempts, zero retries,2456 validation/benchmark calls,120s/Match and60min total. Twelve label tasks/pass represent only8 scientific geometry/side/initiative cells; alias work is charged compatibility evidence outside payoff denominator. Preserve maxPhases100 and597656 conservative total invocation ceiling plus existing tighter limits. No consumed262 route or formation/factory allocation is reused.
4. **RESOLVED: Trace sampling.** Plans01/06 retain full private traces for all24 attempts, covering every inherited sample; review first8 geometry-distinct baseline starts and all failures. Compact outcomes/accounting cover all work; actual storage/throughput/peak memory are recorded under the same timebox. No canonical Chronicle/public replay is created.
