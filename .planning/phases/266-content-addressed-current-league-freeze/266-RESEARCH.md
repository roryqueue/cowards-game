...
1. **Run-head-bound Phase 265 input:** The freeze consumes only `.planning/artifacts/v1.38-phase-265-allocation-v2.json` after authenticating it as the successor bound to the actual run head/result and complete retained journal. Reject the older allocation path and an unbound prospective successor. `verifyRetainedSeriousLeague` remains supporting graph inspection, not the independent empirical-completion verdict. [VERIFIED: `265-07-PLAN.md` Task 3 and `.planning/STATE.md`]
...
1. **Actual empirical head:** Phase 265 Plan 07 Task 3 must produce a complete retained process-valid head, its authenticated allocation successor, and separate independent verification closing LEAG-01–09. None is established here; prospective allocation or injected fixtures are not substitutes. [VERIFIED: `.planning/STATE.md`, `265-VERIFICATION.md`]
