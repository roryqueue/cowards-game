...
**Primary recommendation:** Plan a fail-closed `assemble → independently verify → atomically publish` freeze that can run only after Phase 265 closes; separately specify the precursor-seal-to-final-root join before implementation. [VERIFIED: `266-CONTEXT.md` D-01–D-16; `scripts/lib/v1-38-local-seal.ts:125-136,506-568`]
...
The existing seal's `currentLeagueFreezeRoot` is derived **only** from clean checkout `sourceCommit`, `sourceTree`, and `freezeCarrierIdentity`, before a full league manifest exists; the commitment record also includes it. It is therefore a precommitted checkout/carrier identity, not sufficient as the final Phase 266 manifest-of-leaves root. If the final root binds the commitment and the commitment were redefined to bind that final root, the hash graph would be cyclic. Keep the original commitment immutable and introduce a versioned final freeze envelope that binds both (a) the precursor identity/commitment/custody receipt and (b) the completed league leaves; independently prove their exact source/tuple relationship. Do not rename the old value into the final root or mutate seal v2 history. The precise bridge schema and which existing committed commitment receipt is authoritative are unresolved inputs to planning. [VERIFIED: `scripts/lib/v1-38-local-seal.ts:125-136,506-568`, `.planning/artifacts/v1.38-local-seal-protocol-v2.json`, `266-CONTEXT.md` D-05/D-16]
...
Use a staging/private store and an exclusive immutable final-root publication after all leaf, seal, absence, and projection checks. Repeated `--check` is read-only; `--write` refuses overwrite with different bytes. Publication must not rely on a mutable pointer or silently replace an earlier root. The absence inventory and final root must be checked against the same bounded source/storage snapshot or an equivalent locked/exclusive observation window, then rechecked by Phase 267 before construction. [VERIFIED: `266-CONTEXT.md` D-01/D-08/D-09/D-12; `league/repository.ts` durable exclusive-link pattern; `scripts/evaluate-v1-37-prearchive-proof.ts` write/check precedent]
...
2. **Seal/root identity cycle:** The precursor named `currentLeagueFreezeRoot` is source-only. Keep it distinct from the final league root and verify the join without rewriting the original commitment. [VERIFIED: `v1-38-local-seal.ts:125-136,506-568`]
3. **False absence from partial scan:** Repo import checks omit evidence stores and cannot cover configured private roots. Declare all scan scopes; missing/unreadable scope blocks; seed each artifact class and alternate naming/identity path as a canary. Preserve future-profile policy prose as allowed text. [VERIFIED: `check-v1-38-lab-boundaries.ts`; `266-CONTEXT.md` D-10/D-11]
...
|---|---|---|---|
| — | No unverified package or external-custody claim is recommended. The precise final-root/seal bridge and Phase 265 outputs are unresolved inputs, not assumed facts. | Architecture/Open Questions | Prevents a false release gate. |

## Open Questions / Explicit Phase 265 Dependencies

1. **Exact empirical source:** Which immutable Phase 265 result head, complete retained graph, and independent verification close LEAG-01–09? Current answer: none yet; Plan 07 Task 3 remains pending. Planner must create a blocking input gate, not hard-code a prospective allocation as a result. [VERIFIED: `.planning/STATE.md`, `265-VERIFICATION.md`, absent artifacts]
2. **Original seal commitment and custody:** Which actual restricted commitment record, safe commitment root, and read-only pre-open ledger receipt are authoritative? The checked-in protocol v2 was mechanical and explicitly says `realHoldoutMaterialPresent:false`; do not infer that a later actual commitment is absent without inspecting the operator-configured store at execution. [VERIFIED: `.planning/artifacts/v1.38-local-seal-protocol-v2.json`; `v1-38-local-seal.ts`]
3. **Precursor/final root contract:** Define and review the versioned join from Phase 262's source-commit/tree-derived `currentLeagueFreezeRoot` to the Phase 266 complete-manifest root, preserving original seal bytes and excluding a hash cycle. A missing exact match must block. [VERIFIED: `v1-38-local-seal.ts:125-136,506-568`; `266-CONTEXT.md` D-01/D-05/D-16]
4. **Inventory scope:** Resolve and authenticate every configured private artifact/cache/trace/prompt/model/seal root at execution; an unknown or unreadable root cannot become an “empty” proof. [VERIFIED: `266-CONTEXT.md` D-10/D-11; `check-v1-38-lab-boundaries.ts`]
5. **Preliminary finalist feasibility:** The actual finalist set, declared runtime lanes, exact source hashes, and proof receipts depend on Phase 265 outcome. If none qualifies, freeze the empty allowlist and no readiness claim. [VERIFIED: `266-CONTEXT.md` D-13–D-15; `REQUIREMENTS.md` FRZE-03/CERT-01]
6. **Absence receipt signing:** The context's specific idea calls the absence receipt “signed,” but no Phase 266 signing identity was identified in the inspected code. If an established managed signer exists, bind its identity/signature; otherwise do not describe a content hash as a signature or introduce an ad hoc trust system. The executable inventory/root verification remains mandatory either way. [VERIFIED: `266-CONTEXT.md` Specific Ideas/D-10–D-12; `262-CONTEXT.md` signing-identity discretion]
...
| Full phase gate | Focused freeze/inventory/seal tests, `pnpm exec tsc -b packages/strategy-lab/tsconfig.json --pretty false`, `pnpm exec tsx scripts/check-v1-38-lab-boundaries.ts`, `pnpm exec tsx scripts/check-v1-38-serious-league-boundaries.ts`; read-only real `--check` only after Phase 265 completion. [VERIFIED: existing command paths] |
