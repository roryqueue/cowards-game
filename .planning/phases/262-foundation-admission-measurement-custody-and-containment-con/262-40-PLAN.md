...
must_haves:
  ...
  artifacts:
    - path: .planning/artifacts/v1.38-custody-public-reference.json
      provides: "Bounded public reference to genuine custody evidence, created only after blocking human verification"
  key_links:
...
<task type="checkpoint:human-verify" gate="blocking-human">
  ...
  <action>First run the automated synthetic-mechanics, policy-root, privacy, protected-history, no-authority, no-live-work, formation-absence, and boundary checks. Then pause. The developer must verify that a genuinely separate approved custodian controls the private holdout, discovery/selection identities are immutable and hidden from the study/operator lane, result release is independently controlled, the public reference is opaque and non-reversible, and no Strategy source, memory, SoldierMemory, objective payload, candidate identity, or formation content is exposed. Accept only verifiable external provenance; do not accept self-issued evidence, synthetic mechanics, route 5/A6 materials, summaries, or assertions. If any element is unavailable, leave the checkpoint blocked and do not create the reference. If approved, write only the bounded reference identity and attestations, with `satisfiesSeal01: true` and all downstream authority fields false per D-19/D-20/D-21.</action>
  <acceptance_criteria>
  ...
  <verify>
    <automated>pnpm exec vitest run scripts/lib/v1-38-custody.test.ts scripts/evaluate-v1-38-pre-search-policy.test.ts --maxWorkers=1 &amp;&amp; pnpm exec tsx scripts/check-v1-38-dependency-revision-boundaries.ts --check</automated>
...
|---|---|---|---|---|---|
| T-262-40-01 | Spoofing | Custody attestations | high | mitigate | Blocking human provenance verification; reject self-issued or synthetic substitutes. |
