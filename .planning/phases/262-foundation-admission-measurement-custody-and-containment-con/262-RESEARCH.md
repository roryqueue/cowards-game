...
```mermaid
flowchart TD
  A["Maintainer invokes v1.38 foundation check"] --> J["Admission join"]
  J -->|missing, stale, drift, mismatch| STOP["stop_integrity_foundation<br/>no authoritative output"]
  J -->|exact pass| R["Supervised historical matrix reproduction"]
  R -->|mismatch or runtime/system failure| STOP
  R -->|exact fixture receipt| C["Profile-neutral calibration"]
  C --> F["Freeze study + budget + classifier + claim contract"]
  H["Custodian private store<br/>holdout preimages + secret material"] --> K["Profile-agnostic commitment only"]
  K --> F
  F --> B["Containment monitor"]
  B -->|forbidden artifact/import found| STOP
  B -->|negative proof passes| ROOT["Immutable Phase 262 root<br/>admission + reproduction + contract + custody public ref + containment proof"]
  ROOT --> P263["Phase 263 may begin"]
  H -. "one authorized opening only in Phase 269" .-> RECEIPT["Bounded safe receipt"]
  RECEIPT -. "no preimages/evaluator internals" .-> REPORT["Later orthogonal report fields"]
```
