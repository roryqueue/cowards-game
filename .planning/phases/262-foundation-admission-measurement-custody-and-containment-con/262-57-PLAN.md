...
must_haves:
  truths:
    - "Only the main orchestrator may consume exact checked A8/B8 authorization-v8/seal-v8 after proving clean ownership, zero active helpers/executors, fresh authority bytes, and absence of every route-ordinal-7 live destination."
  ...
  key_links:
    - from: .planning/artifacts/v1.38-successor-source-seal-v8.json
      ...
      via: "Exact A8/B8 authority-v8 custody, main-only ownership, fresh-destination check, and one atomic publication binding context:v11 plus preflight consumption"
  ...
  prohibitions:
    - statement: "Do not execute from a helper/subagent/worktree, with active executors, from a dirty or non-main context, or without exact A8/B8 and fresh destination checks."
      status: locked
    - statement: "Do not read, create, or claim obsolete authorization-v7/seal-v7 future paths; route ordinal remains 7 while authority/seal schema and paths are v8."
      status: locked
...
<objective>
Consume exact A8/B8 authorization-v8 authority once from the main orchestrator, execute unchanged route ordinal 7 with v11/v12 artifacts, and seal one truthful permanent terminal.
...
<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 1: Confirm exact A8/B8 and exclusive main-orchestrator ownership</name>
  <read_first>
    - .planning/phases/262-foundation-admission-measurement-custody-and-containment-con/262-56-SUMMARY.md
    - .planning/artifacts/v1.38-plan-262-56-authorization-v8.json
    - .planning/artifacts/v1.38-successor-source-seal-v8.json
  ...
  <acceptance_criteria>
    - Exact A8/B8 custody and authorization-v8 bytes are fresh and checked immediately before the first write.
...
|---|---|
| main orchestrator -> one-shot authority | Only exact clean exclusive ownership may consume B8 while route ordinal remains 7. |
...
|---|---|---|---|---|---|
| T-262-57-01 | Spoofing | A8/B8/main ownership | high | mitigate | Immediate full custody/readiness check, review-v2-aware authority validation, and zero-helper gate. |
...
<verification>
- Exact A8/B8 and clean main-only ownership pass immediately; live-destination absence gates unchanged route ordinal 7 execution, while any initial obstruction routes only to the separate checked pre-start disposition.
