...
<task type="checkpoint:human-action" gate="blocking-human">
  ...
  <how-to-verify>
    ...
    3. Return only `consume-route-ordinal-7-once sourceA8=<FULL_A8_OID> sourceB8=<FULL_B8_OID>` with the exact checker-derived OIDs; any interruption requires rerunning the entire read-only gate and regenerating the phrase.
  </how-to-verify>
  <resume-signal>Operator must return the exact checker-rendered consumption phrase with full A8/B8 OIDs; no other phrase or auto-advance signal resumes this checkpoint.</resume-signal>
  <acceptance_criteria>
...
|---|---|---|---|---|---|
| T-262-57-01 | Spoofing | A8/B8/main ownership/operator consumption | high | mitigate | Immediate full custody/readiness check, review-v2-aware authority validation, zero-helper gate, and non-auto-bypassable exact consumption phrase with full checker-derived A8/B8 OIDs. |
