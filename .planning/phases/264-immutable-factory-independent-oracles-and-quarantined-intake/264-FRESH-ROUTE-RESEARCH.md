...
### Implementation correction — 2026-09-14

The installed CLI's ordinary `exec --json` completion stream does not document the required reported-model identity. The selected implementation therefore uses the same authenticated local CLI's **app-server stdio protocol**, whose installed generated response schemas report effective model/provider, approval policy, sandbox and instruction sources at thread start, and actual usage at turn completion. Read-only help/schema inspection and fake-process tests establish the source contract; no live handshake or inference has yet established operational availability. The exact inspected executable and sanitized environment, a private non-repository disclosed-packet directory, pre-charge effective settings checks, and observed bounded child cleanup must all pass the independent source recheck. This technical transport correction does not enlarge the approved authoring budget or information boundary. See [official app-server documentation](https://learn.chatgpt.com/docs/app-server).

The prescribed calibration-control materializer was moved forward from Plan08 Task1 into the Plan07 preparation repair because enforcing the twelve-slot graph requires it. It reopens the three named base emissions and rederives every control's exact source/provenance. Controls have a local transform provider and `calibration_only` ingestion status; their old fingerprint projection remains `mechanics_only`, never an independent producer. Supervised control observations, numeric thresholds, first-valid model-attempt linkage and the 90-minute runner guard remain Plan08 obligations, not source-only accomplishments.

Run authoring outside Match/search execution in four isolated, non-interactive authoring contexts at most. Each context receives only the disclosed current-rules/Strategy ABI packet and task instructions. It must not read repository source, old-rules submissions, other oracle cores, private results, holdout material, secrets, or evaluator state. The initial recommendation used the CLI's non-interactive JSON mode as documented at [official non-interactive mode documentation](https://learn.chatgpt.com/docs/non-interactive-mode); the implementation correction above supersedes that transport choice. This research performed no authoring request or availability probe.
