...
function validateCapacityCosts(input: LeagueCapacityPlanInput, allocation: ProspectiveLeagueExecutionAllocation): void {
  ...
  for (const [index, row] of input.costs.entries()) {
    ...
    if (!exact(m, ["sourceRoot", "witnessRoots", "sampleUnits", "measuredBytes", "measuredRecords", "projectedUnits"]) || m.sourceRoot !== input.sourceRoot || !Array.isArray(m.witnessRoots) || !m.witnessRoots.length || !m.witnessRoots.every(isRoot) || new Set(m.witnessRoots).size !== m.witnessRoots.length || ![m.sampleUnits, m.measuredBytes, m.projectedUnits].every((v) => int(v) && v > 0) || !int(m.measuredRecords) || (row.category === "filesystem" ? m.measuredRecords !== 0 : m.measuredRecords < 1) || m.projectedUnits !== projectedUnits || row.measurementRoot !== labRoot("league-data-only-capacity-measurement-v1", { category: row.category, ...m })) fail("CAPACITY_MEASUREMENT")
