...
import { createFactoryFingerprintEvidence } from "../factory/fingerprint.js"
...
export const importedCandidateFixture = async (slot: number) => {
  ...
  const mechanicsOnly = ![1, 3, 5].includes(slot)
  const fingerprintValue = { proposalRoot: proposal.root, validationRoot: validation.root, supervisionReceiptRoot: receipt.root, producerIdentity: mechanicsOnly ? "materializeFactoryCalibrationControl" as const : "emitTacticalFactoryPacket" as const, origin: mechanicsOnly ? "calibration-control" as const : "tactical-oracle" as const, evidenceClass: mechanicsOnly ? "mechanics_only" as const : "real_producer" as const, producerArtifactRoot: mechanicsOnly ? null : put(producer), authorshipRoots: [root("a")], lineageNodes: [{ root: proposal.root, artifactRoot: root("b"), parents: [] }], dependencyNodes: [{ root: sourceRoot, artifactRoot: sourceRoot, dependencies: [] }], matchupResponses: [{ supervisionReceiptRoot: receipt.root, conditionRoot: root("c"), opponentRoot: root("d"), side: "bottom" as const, initialInitiative: true, outcome: "draw" as const, responseRoot: root("e") }], counterfactualPairs: [{ leftRoot: proposal.root, rightRoot: proposal.root, relation: "borderline" as const }], failureModes: ["accepted" as const] }
  const fingerprint = mechanicsOnly ? createFactoryFingerprintEvidence(fingerprintValue) : rooted("factory-fingerprint-evidence-v1", { privacy: "private_offline", ...fingerprintValue })
  ...
  return { input, candidateAdmission: importAssessedFactoryCandidate(input), factoryRepository: repository, fingerprintArtifactRoot, fingerprint, put, closure: { factoryRepository: repository, candidatePublicationArtifactRoot: publicationArtifactRoot, sourceArtifactRoot: admitted.artifacts.source, packetArtifactRoot: admitted.artifacts.packet, proposalArtifactRoot: admitted.artifacts.proposal, validationArtifactRoot: admitted.artifacts.validation }, importedAssessment: { maxBytes: input.maxBytes, maxRecords: input.maxRecords, verifyRetainedAssessment } }
}
