...
const statusPayload = (
  ...
  reasonCode: domain === "certificate-revocation" ? "evidence-invalid" : "recertified",
...
describe("authenticated runtime evidence authority controls", () => {
  it("rejects forged, unknown-key, wrong-domain, stale, partial, and lane-mismatched controls with zero rows", async () => {
    ...
    const candidates = [
      ...
      envelope(lanePayload({ domain: "certificate-revocation" })),
...
describePostgres("PostgreSQL append-only runtime evidence authority schema", () => {
  ...
  const seedCertificate = async (
    ...
    const common = [
      "passed", "containment", trustRoot.producerId, trustRoot.keyId,
      trustRoot.trustDomain, "runtime-evidence-attestation-v1", "command",
      rawHash, "corpus", rawHash, "policy", rawHash, "node", "24",
      "typescript", "6", "json", "1", lane.artifactId, rawHash,
      laneIdentityHash, lane.semanticTupleId, rawHash, rawHash, rawHash,
      "runtime-certificate-v1", rawHash, "fixture:generation:1", lane,
    ...
    await pool.query(
      `insert into runtime_evidence_certificates
       ...
       values ($1,'containment','runtime-certificate-v1',$2,'passed',$3,'passed',
         ...
         '2026-07-12T00:00:00.000Z','2026-08-12T00:00:00.000Z')`,
      [id, rawHash, attestationId, trustRoot.producerId, rawHash, lane.artifactId, laneIdentityHash, lane],
    )
  }
  ...
  it("appends authenticated disable and compensating enable controls and rejects mutation", async () => {
    ...
    await expect(
      pool.query("update runtime_evidence_lane_controls set reason_code = 'changed'"),
  ...
  it("appends exact revocation and cycle-free supersession and rejects replay, conflict, self, and cycle cases with zero rows", async () => {
    await seedCertificate("certificate:fixture:one", sha256("certificate:one"), "one")
    await seedCertificate("certificate:fixture:two", sha256("certificate:two"), "two")
    const revocationEnvelope = envelope(statusPayload("certificate-revocation"))
