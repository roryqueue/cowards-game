  ...
  it("builds one strict four-lane, twelve-run, D-01..D-16 proof", () => {
    const proof = buildV137ExecutableConformanceProof(
      ...
      passingGates().map((gate, index) => ({
        ...gate,
        command: [
          "pnpm exec vitest run --maxWorkers=1 --no-file-parallelism packages/spec/src/runtime-conformance-certificate-v1-17.test.ts packages/spec/src/runtime-semantic-receipt-v1-18.test.ts packages/spec/src/runtime-execution-service-v1-18.test.ts packages/spec/src/runtime-budget-capabilities-v1-18.test.ts packages/persistence/src/runtime-evidence-authority-publisher.test.ts packages/persistence/src/complete-match.test.ts packages/runtime-js/src/candidate-process-runner.test.ts packages/runtime-js/src/executor.test.ts apps/runtime-service/src/execute-match-v1-18.test.ts scripts/sign-v1-37-language-conformance-certificate.test.ts",
          "pnpm go:parity",
          "pnpm contract:check",
