...
const exactDockerAbsentDiagnostic = (status: number | null, signal: NodeJS.Signals | null, stdout: string, stderr: string, name: string): boolean => signal === null && status === 1 && ((stdout === "" && stderr === `Error: No such object: ${name}\n`) || (stdout === "\n" && stderr === `error: no such object: ${name}\n`))
...
export const writeLeanDirectWorkerLifecycleDiagnosticV1 = (repoRoot: string, explicitSourceRef: string): LeanWorkerLifecycleDiagnosticV1 => {
  ...
  try {
    ...
    const absent = diagnosticDocker(["inspect", name]); if (!exactDockerAbsentDiagnostic(absent.status, absent.signal, String(absent.stdout), String(absent.stderr), name)) throw new TypeError("docker_unavailable")
