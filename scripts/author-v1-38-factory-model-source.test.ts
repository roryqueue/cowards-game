...
describe("operational factory authoring", () => {
  ...
  it("parses documented JSONL, admits source, and rederives the exact V2 bundle", () => {
    ...
    const bundle = createFrozenModelBundleV2FromAuthorAttempt(ledger, allocation, launch, start); expect(bundle.provenance).toMatchObject({ requestedModelId: "gpt-5.6-sol", reportedModelId: "gpt-5.6-sol", actualUsage: terminal.usage }); expect(bundle.provenance.rawResponseRecord.bodyUtf8).toBe(new TextDecoder().decode(stdout))
  ...
  it("uses schema-verified app-server identity before charging and links its raw turn into V2", async () => {
    const parent = directory(), auth = join(parent, "native-auth.json"); writeFileSync(auth, "not-read-by-author-code")
    const times = [1_000, 1_010, 1_060, 1_070, 1_080]
    const result = await runFactoryAppServerAuthorAttempt({ allocation: createFactoryAuthoringAllocation(), packetBytes: packet, packetRoot, disclosedDirectory: join(parent, "disclosed"), stateDirectory: join(parent, "state"), existingAuthFile: auth, ledgerDirectory: join(parent, "ledger"), model: "gpt-5.6-sol", modelProvider: "openai", frozenSettings, capability, clock: () => times.shift()!, transportFactory: async (options) => { expect(options).toMatchObject({ requestedModel: "gpt-5.6-sol", requestedProvider: "openai", cwd: join(parent, "disclosed") }); return { threadId: "thread-1", reportedModel: "gpt-5.6-sol", close() {}, async startTurn() { return { sourceMessage: JSON.stringify({ source }), usage: { inputTokens: 12, cachedInputTokens: 2, outputTokens: 8, reasoningOutputTokens: 1, totalTokens: 20 }, reportedModel: "gpt-5.6-sol", rawJsonl: new TextEncoder().encode('{"method":"thread/tokenUsage/updated"}\n') } } } } })
