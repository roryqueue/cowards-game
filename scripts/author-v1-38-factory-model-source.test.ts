...
describe("operational factory authoring", () => {
  ...
  it("parses documented exec JSONL but refuses a V2 bundle without raw reported identity", () => {
    ...
    expect(() => createFrozenModelBundleV2FromAuthorAttempt(ledger, allocation, launch, start)).toThrow("MODEL_RAW_RESPONSE_IDENTITY")
  ...
  it("uses schema-verified app-server identity before charging and links its raw turn into V2", async () => {
    ...
    const rawJsonl = new TextEncoder().encode([
      { jsonrpc: "2.0", id: 2, result: { thread: { id: "thread-1" }, model: "gpt-5.6-sol", modelProvider: "openai", cwd: join(parent, "disclosed"), sandbox: { type: "readOnly", networkAccess: false }, instructionSources: [] } },
