...
type CurrentSelection = typeof CURRENT_SEMANTIC_AUTHORITY_GENERATED.selection
...
export const buildV137ArenaSetAuthorityArtifacts = (
  ...
  assertExactSource(
    "Phase-259 current selection",
    currentSelection,
    CURRENT_SEMANTIC_AUTHORITY_GENERATED.selection,
  )
  if (
    (currentSelection as CurrentSelection).semanticAuthorityKey !==
      "runtime-v1.17" ||
    (currentSelection as CurrentSelection).runtimeAbiVersion !==
      "strategy-runtime-abi-v1.17" ||
    (currentSelection as CurrentSelection).conformanceCertificateVersion !==
      "runtime-conformance-certificate-v1.17"
  ) {
    throw new Error("Current selector is not the exact Phase-259 selection")
  }
  ...
  return Object.freeze({
    ...
    outputSha256: sha256(
      stableJson({
        ...
        currentSelection: source.currentSelection,
      }),
    ),
  })
}
...
export const renderV137ArenaSetAuthorityArtifacts = (
  overrides: V137ArenaSetAuthoritySourceOverrides = {},
): Readonly<Record<(typeof v137ArenaSetAuthorityOutputPaths)[number], string>> => {
...
const defaultIo: V137ArenaSetAuthorityGeneratorIo = {
  readOutput: (relativePath) => readFileSync(path.join(repoRoot, relativePath), "utf8"),
...
export const runV137ArenaSetAuthorityGenerator = (
  ...
  if (check) {
    ...
    if (findings.length > 0) {
      throw new Error(`Go arena/Set authority check failed: ${findings.join(", ")}`)
    }
  }
