...
/**
 * Darwin 25 exposes neither fexecve(2) nor execveat(2), and an executable
 * retained on a descriptor cannot be launched through /dev/fd (exec returns
 * EACCES).  This bootstrap therefore makes the reduced assurance explicit:
 * it detects accidental/path-replacement drift, but it does not claim to
 * resist a hostile concurrent process running as the same uid.  That is the
 * exact single-operator local-seal boundary used by Phase 262.
 */
export const V138_PRIVATE_NATIVE_EXECUTION_ASSURANCE_V2 = Object.freeze({
...
export type V138PrivateNativeV2 = Readonly<{
  directory: string
  executable: string
  executableSha256: string
  compilerSha256: string
  sourceSha256: string
  executionAssurance: typeof V138_PRIVATE_NATIVE_EXECUTION_ASSURANCE_V2
  cleanup: () => void
}>

/**
 * Copies reviewed compiler bytes from a retained descriptor into a private
 * 0700 directory and detects drift around each pathname launch. This is a
 * single-operator local seal, not a hostile-same-uid isolation boundary; see
 * V138_PRIVATE_NATIVE_EXECUTION_ASSURANCE_V2.
 */
export const compileV138PrivateNativeV2 = (input: Readonly<{
  source: string
  ...
  try {
    ...
    for (const output of [executable, reproduced]) {
      const preLaunchDescriptor = openSync(
        compiler,
        constants.O_RDONLY | (constants.O_NOFOLLOW ?? 0),
      )
      const beforeLaunch = fstatSync(preLaunchDescriptor)
      const compilerPathBefore = lstatSync(compiler)
      if (
        beforeLaunch.dev !== compilerPathBefore.dev ||
        beforeLaunch.ino !== compilerPathBefore.ino
      ) {
        closeSync(preLaunchDescriptor)
      ...
      const compilerPathAfter = lstatSync(compiler)
      closeSync(preLaunchDescriptor)
      if (
        beforeLaunch.dev !== compilerPathAfter.dev ||
        beforeLaunch.ino !== compilerPathAfter.ino ||
        sha256(readFileSync(compiler)) !== EXPECTED_PRIVATE_CLANG_SHA256
      )
