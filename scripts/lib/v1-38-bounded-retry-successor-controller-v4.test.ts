...
import {
  ...
  V138_SUCCESSOR_CONTROLLER_V4_CLI,
  V138_SUCCESSOR_CONTROLLER_V4_OPERATIONS,
} from "./v1-38-bounded-retry-successor-controller-v4.js"

const run = (mode: string) =>
  spawnSync(
    process.execPath,
    ["--import", "tsx", V138_SUCCESSOR_CONTROLLER_V4_CLI, mode],
...
describe("CR-01 controller-owned successor mutation closure", () => {
  it("exports only non-writing constants and source checks", async () => {
    const controller =
      await import("./v1-38-bounded-retry-successor-controller-v4.js")
    ...
    expect(
      checkV138SuccessorControllerV4Source(V138_SUCCESSOR_CONTROLLER_V4_CLI),
    ).toBe(true)
  })
  ...
  it("runs the complete write protocol only inside its controller-created temporary root", () => {
    ...
    expect(output.operations).toEqual(V138_SUCCESSOR_CONTROLLER_V4_OPERATIONS)
