...
export type V138RetryV3RouteIdentity = `route:v3:${0 | 1 | 2}`
export type V138RetryV3PreflightIdentity = `preflight:v3:${number}`
export type V138RetryV3CalibrationIdentity =
  `calibration:v3:${0 | 1 | 2}:${number}`
export type V138RetryV3ReproductionIdentity = `reproduction:v3:${number}`
...
export const V138_BOUNDED_RETRY_V3_POLICY = deepFreeze({
  schemaVersion: "retry-envelope:v3" as const,
...
const routes = Array.from(
  { length: 3 },
  (_, ordinal) => `route:v3:${ordinal}` as V138RetryV3RouteIdentity,
)
const preflights = Array.from(
  { length: 12 },
  (_, ordinal) => `preflight:v3:${ordinal}` as V138RetryV3PreflightIdentity,
)
const calibrations = routes.flatMap((_, routeOrdinal) =>
  Array.from(
    { length: 8 },
    (_, attemptOrdinal) =>
      `calibration:v3:${routeOrdinal}:${attemptOrdinal}` as V138RetryV3CalibrationIdentity,
  ),
)
const reproduction = Array.from(
  { length: 540 },
  (_, ordinal) => `reproduction:v3:${ordinal}` as V138RetryV3ReproductionIdentity,
)

export const V138_BOUNDED_RETRY_V3_IDENTITIES = deepFreeze({
  routes,
  preflights,
  calibrations,
  reproduction,
...
export const V138_BOUNDED_RETRY_V3_PROTECTED_HISTORY = deepFreeze({
...
export const checkV138ProtectedHistoryV3 = (
  value: unknown,
): typeof V138_BOUNDED_RETRY_V3_PROTECTED_HISTORY => {
  if (canonical(value) !== canonical(V138_BOUNDED_RETRY_V3_PROTECTED_HISTORY)) {
    return fail("V138_RETRY_V3_PROTECTED_HISTORY_INVALID")
  }
  return value as typeof V138_BOUNDED_RETRY_V3_PROTECTED_HISTORY
}

export const V138_BOUNDED_RETRY_V3_PATHS = deepFreeze({
  reproduction:
  ...
  lock: ".planning/artifacts/v1.38-current-matrix-retry-journal-v3.jsonl.lock",
...
export const requireV138RetryV3DestinationAbsent = (
  root: string,
  ...
  if (
    ...
    return fail("V138_RETRY_V3_DESTINATION_UNSAFE")
  }
...
export const requireV138RetryV3ReproductionAbsent = (root: string): true => {
  try {
    return requireV138RetryV3DestinationAbsent(
      root,
...
export interface V138InactiveRetryV3Envelope {
  readonly schemaVersion: "retry-envelope:v3"
  ...
  readonly policy: typeof V138_BOUNDED_RETRY_V3_POLICY
...
export const createV138InactiveRetryV3Envelope = (
  input: Readonly<{
...
): Readonly<V138InactiveRetryV3Envelope> => {
  if (
    ![
      input.sourceRoot,
      input.reviewRoot,
    ...
    input.protectedHistoricalIdentities.some(
      (identity) =>
        ...
        identity.length === 0 ||
        routes.includes(identity as V138RetryV3RouteIdentity) ||
        preflights.includes(identity as V138RetryV3PreflightIdentity) ||
        calibrations.includes(identity as V138RetryV3CalibrationIdentity) ||
        reproduction.includes(identity as V138RetryV3ReproductionIdentity),
    )
  ) {
  ...
  const body = {
    ...
    policy: V138_BOUNDED_RETRY_V3_POLICY,
    counters: {
  ...
  return deepFreeze({
    ...body,
    envelopeRoot: sha256(`v138-retry-envelope-v3\0${canonical(body)}`),
  })
}

export const checkV138InactiveRetryV3Envelope = (
  value: unknown,
): Readonly<V138InactiveRetryV3Envelope> => {
  const candidate = value as V138InactiveRetryV3Envelope
  try {
    const expected = createV138InactiveRetryV3Envelope({
...
type ReservePreflight = Readonly<{
  ...
  identity: V138RetryV3PreflightIdentity
  owner: string
}>
...
type ReserveRoute = Readonly<{
  kind: "reserve_route"
  identity: V138RetryV3RouteIdentity
  owner: string
  preflightIdentity: V138RetryV3PreflightIdentity
}>
...
export type V138RetryV3JournalEvent =
  | ReservePreflight
  | ObservePreflight
  | ReserveRoute
  | ReserveCalibration
...
export type V138RetryV3JournalRecord = Readonly<
  V138RetryV3JournalEvent & {
    schemaVersion: "v1.38-bounded-retry-journal-record-v3"
    ordinal: number
...
const GENESIS_ROOT = sha256("v138-bounded-retry-journal-genesis-v3")

type ReplayState = {
  preflightReservations: Map<V138RetryV3PreflightIdentity, string>
  preflightObservations: Map<V138RetryV3PreflightIdentity, number>
  routeReservations: Map<V138RetryV3RouteIdentity, string>
  routePreflights: Map<V138RetryV3RouteIdentity, V138RetryV3PreflightIdentity>
  calibrationReservations: Map<
    V138RetryV3RouteIdentity,
    readonly V138RetryV3CalibrationIdentity[]
  >
  calibrationTerminals: Map<V138RetryV3RouteIdentity, FinishCalibration>
  reproductionRoute: V138RetryV3RouteIdentity | null
...
const replay = (
  ...
  for (let ordinal = 0; ordinal < records.length; ordinal += 1) {
    ...
    if (
      record.schemaVersion !== "v1.38-bounded-retry-journal-record-v3" ||
      ...
      !isSha256(recordRoot) ||
      recordRoot !== sha256(`v138-retry-journal-record-v3\0${canonical(body)}`)
    ) {
    ...
    try {
      ...
      applyEvent(state, event as V138RetryV3JournalEvent, atMilliseconds)
    } catch {
      fail("V138_RETRY_JOURNAL_CHAIN_INVALID")
    }
...
export const appendV138RetryV3JournalRecord = (
  records: readonly V138RetryV3JournalRecord[],
  event: V138RetryV3JournalEvent,
  atMilliseconds: number,
  envelopeRoot: V138RetrySha256,
): readonly V138RetryV3JournalRecord[] => {
  ...
  const body = {
    schemaVersion: "v1.38-bounded-retry-journal-record-v3" as const,
  ...
  const record = deepFreeze({
    ...body,
    recordRoot: sha256(`v138-retry-journal-record-v3\0${canonical(body)}`),
  }) as V138RetryV3JournalRecord
  return Object.freeze([...records, record])
}

export interface V138DerivedRetryV3State {
  readonly schemaVersion: "v1.38-bounded-retry-derived-state-v3"
  ...
  readonly nextPreflightIdentity: V138RetryV3PreflightIdentity | null
  readonly nextRouteIdentity: V138RetryV3RouteIdentity | null
...
export const deriveV138RetryV3State = (
  envelopeValue: unknown,
  records: readonly V138RetryV3JournalRecord[],
): Readonly<V138DerivedRetryV3State> => {
  const envelope = checkV138InactiveRetryV3Envelope(envelopeValue)
  ...
  const body = {
    schemaVersion: "v1.38-bounded-retry-derived-state-v3" as const,
  ...
  return deepFreeze({
    ...body,
    stateRoot: sha256(`v138-retry-derived-state-v3\0${canonical(body)}`),
  })
}

export const encodeV138RetryV3CanonicalJson = (value: unknown): string =>
  canonical(value)
