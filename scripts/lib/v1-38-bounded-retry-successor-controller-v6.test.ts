...
import {
  checkV138SuccessorControllerV4Source,
  V138_SUCCESSOR_CONTROLLER_V6_CLI,
  V138_SUCCESSOR_CONTROLLER_V6_OPERATIONS,
} from "./v1-38-bounded-retry-successor-controller-v6.js"

const run = (mode: string) =>
  spawnSync(
    process.execPath,
    ["--import", "tsx", V138_SUCCESSOR_CONTROLLER_V6_CLI, mode],
    {
      cwd: process.cwd(),
      encoding: "utf8",
      env: {
        ...process.env,
        V138_TEST_PARENT_LOADER_SENTINEL: "must-not-cross-child-boundary",
      },
    },
  )
...
describe("CR-01 controller-owned successor mutation closure", () => {
  it("exports only non-writing constants and source checks", async () => {
    const controller =
      await import("./v1-38-bounded-retry-successor-controller-v6.js")
    const pair = await import("./v1-38-durable-pair-successor-v2.js")
    const lifecycle =
      await import("./v1-38-restartable-lifecycle-successor-v2.js")
    ...
    for (const keys of [
      ...
      expect(keys.join(" ")).not.toMatch(
        /(?:write|publish|apply|mutate|run|invoke|worker)/iu,
      )
    }
    expect(
  ...
  it("runs the complete write protocol only inside its controller-created temporary root", () => {
    ...
    expect(output.operations).toEqual(V138_SUCCESSOR_CONTROLLER_V6_OPERATIONS)
    expect(output.nativeExecutionAssurance).toEqual({
    ...
    expect(output.pairMembers).toEqual([
      '{"authority":false}\n',
    ...
    expect(output.sharedIntentConflicts).toBe(13)
    expect(output.crashRecoveries).toBe(16)
  ...
  it.each(["--source-check"])(
    "allows the non-live %s CLI mode",
    (mode) => {
  ...
  it.each([
    ...
    "--activate",
    "--unknown",
  ])("fails closed for forbidden CLI mode %s", (mode) => {
    const result = run(mode)
