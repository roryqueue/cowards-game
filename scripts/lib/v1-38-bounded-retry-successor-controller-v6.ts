 ...
 * installed in a predictable cache. Darwin has no acceptable descriptor-exec
 * primitive, so this controller intentionally inherits the explicit
 * single-operator/no-hostile-same-uid boundary exported by the bootstrap; it
 * does not claim pathname-launch replacement resistance.
 */
...
> => {
  const root = mkdtempSync(path.join(tmpdir(), "v138-successor-controller-v6-"))
  try {
    ...
    return Object.freeze({
      operations: V138_SUCCESSOR_CONTROLLER_V6_OPERATIONS,
      nativeExecutionAssurance: V138_PRIVATE_NATIVE_EXECUTION_ASSURANCE_V2,
      acceptedCells: 0,
...
if (process.argv[1] === V138_SUCCESSOR_CONTROLLER_V6_CLI) {
