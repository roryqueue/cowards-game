...
import { createFactoryGraphNodeArtifact, createLeagueAuthorizedFactoryFingerprintEvidence as issueLeagueFingerprint, deriveFactoryFingerprints, issueFactoryPairedCommitment, verifyRetainedLeagueFactoryFingerprints, type IssuedFactoryPairedCommitment } from "../../packages/strategy-lab/src/factory/fingerprint.js"
...
export const produceLeagueResponse = async (input: LeagueResponseProductionInput) => {
  ...
  try {
    ...
    let primaryReceipt: FactorySupervisionReceipt | null = null
    const pairedCommitments: IssuedFactoryPairedCommitment[] = [], comparisons: any[] = [], scores: Array<{ seed: string; opponentRoot: LabRoot; numerator: number; denominator: number; evidenceRoots: LabRoot[] }> = []
    const arenas = CANONICAL_ARENA_CATALOG_V1_37.arenas.filter((arena) => arena.status === "active" && arena.schedulable)
    if (arenas.length !== 2) return fail("RESPONSE_ARENAS")
      ...
      for (const seed of allocation.seedBlocks) {
        ...
        for (const condition of conditions.filter((row) => row.opponentRoot === opponent.candidateRoot && row.seed === seed)) {
          ...
          try {
            ...
            if (purpose === "score") { primaryReceipt ??= receipt; pairedCommitments.push(issueFactoryPairedCommitment(receipt)); scoreEvidence.push(resultRoot) }
    ...
    const evidence = createLeagueAuthorizedFactoryFingerprintEvidence({ repository: input.repository, allocationArtifactRoot, startArtifactRoot: input.startArtifactRoot, supervisionReceipt: receipt, value: { proposalRoot: proposal.root, validationRoot: validation.root, supervisionReceiptRoot: receipt.root, producerIdentity: ingestion.producerIdentity, origin: ingestion.origin, evidenceClass: "real_producer", producerArtifactRoot: author.ingestionArtifactRoot, authorshipRoots: [ingestion.root], lineageNodes, dependencyNodes, matchupResponses: pairedCommitments.map((row) => { if (row.matchup.status !== "verified") return fail("RESPONSE_MATCHUP"); return { supervisionReceiptRoot: row.supervisionReceiptRoot, conditionRoot: row.matchup.conditionRoot, opponentRoot: row.matchup.opponentRoot, side: row.matchup.side, initialInitiative: row.matchup.initialInitiative, outcome: row.outcome, responseRoot: labRoot("league-response-execution-v1", row.execution) } }), counterfactualPairs: comparisons.map((entry) => ({ leftRoot: proposal.root, rightRoot: entry.opponentProposalRoot, relation: entry.relation === "unresolved" ? "borderline" : entry.relation })), failureModes: ["accepted"] } })
    const fingerprintArtifactRoot = publishFactoryArtifact(input.repository, encode(evidence)), independence = deriveFactoryFingerprints({ repository: input.repository, supervisionReceipt: receipt, pairedCommitments, evidence, evidenceArtifactRoot: fingerprintArtifactRoot })
...
export const verifyRetainedLeagueResponse = (input: {
  ...
  records: { get(root: LabRoot): { kind: string; value: any; links: readonly LabRoot[] } | undefined; entries(): IterableIterator<[LabRoot, { kind: string; value: any; links: readonly LabRoot[] }]>; linked?(kind: string, root: LabRoot): readonly LabRoot[]; matches?(kind: "response-match-start" | "response-match-result", parentStartRoot: LabRoot): readonly { root: LabRoot; ordinal: number }[] }
}) => {
  ...
  const conditions = enumerateLeagueResponseConditions(allocation, opponents.map((opponent) => opponent.candidateRoot)), all = [...input.records.entries()]
  const results = new Map<number, LabRoot>(), charges = new Map<number, LabRoot>()
  const linked = (kind: string, root: LabRoot) => input.records.linked
    ? input.records.linked(kind, root).map((recordRoot) => [recordRoot, input.records.get(recordRoot)!] as const)
    : all.filter(([, node]) => node.kind === kind && node.links.includes(root))
  if (input.records.matches) {
    for (const { root, ordinal } of input.records.matches("response-match-result", produced.admission.attemptStart.root)) { if (results.has(ordinal)) return fail("RETAINED_RESPONSE_COVERAGE"); results.set(ordinal, root) }
    for (const { root, ordinal } of input.records.matches("response-match-start", produced.admission.attemptStart.root)) { if (charges.has(ordinal)) return fail("RETAINED_RESPONSE_COVERAGE"); charges.set(ordinal, root) }
  } else for (const [root, node] of all) {
    if (node.kind === "response-match-result") { const { ordinal, parentStartRoot } = node.value.matchCharge; if (parentStartRoot === produced.admission.attemptStart.root) { if (results.has(ordinal)) return fail("RETAINED_RESPONSE_COVERAGE"); results.set(ordinal, root) } }
    if (node.kind === "response-match-start") { const { ordinal, parentStartRoot } = node.value; if (parentStartRoot === produced.admission.attemptStart.root) { if (charges.has(ordinal)) return fail("RETAINED_RESPONSE_COVERAGE"); charges.set(ordinal, root) } }
  }
  if (results.size !== conditions.length || charges.size !== conditions.length || produced.matchCount !== conditions.length) return fail("RETAINED_RESPONSE_COVERAGE")
  const thresholdRecord = read(input.threshold.repository, input.threshold.artifactRoot), fit = freezeNumericCalibrationThreshold(thresholdRecord.controls)
  ...
  for (const condition of conditions) {
    const recordRoot = results.get(condition.ordinal) ?? fail("RETAINED_RESPONSE_CONDITION"), node = input.records.get(recordRoot) ?? fail("RETAINED_RESPONSE_CONDITION"), value = node.value, { arenaIndex: _arenaIndex, ...fields } = condition
    ...
    const chargeRoot = charges.get(condition.ordinal), charge = chargeRoot && input.records.get(chargeRoot)
    if (!chargeRoot || !charge || !same(charge.value, value.matchCharge) || !node.links.includes(chargeRoot) || value.match.matchId !== `league-response-${chargeRoot.slice(7, 31)}` || value.match.seed !== condition.seed || value.match.arenaVariant.semanticGeometryHash !== condition.arena) return fail("RETAINED_RESPONSE_CHARGE")
    ...
    const rawInvocations = linked("response-runtime-invocation", chargeRoot)
    ...
    for (const [artifactRoot, candidate, playerId] of [[value.candidateReceiptArtifactRoot, measured, "league-response-candidate"], [value.opponentReceiptArtifactRoot, opposing, "league-response-opponent"]] as const) {
      ...
      const traces = stored.records.filter((row) => row.kind === "trace").map((row) => row.value as any), invocations = rawInvocations.filter(([, row]) => same(row.value.originalEvidence.identity, metadata.candidateIdentity))
      ...
      const cleanups = linked("response-runtime-cleanup", chargeRoot).filter(([, row]) => same(row.value.identity, metadata.candidateIdentity))
