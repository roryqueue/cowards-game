...
const nativeSource = path.resolve(
  sourceDirectory,
  "../native/v1-38-successor-transaction-helper-v3.c",
)
const EXPECTED_NATIVE_SOURCE_SHA256 =
  "7664fe6f95b984164b60d24b4558107b700cfc348786a6a0d3897eeb3eb5124c"
...
const compileOneShotNative = (
  ...
  if (
    sha256Hex(sourceBefore) !== EXPECTED_NATIVE_SOURCE_SHA256 ||
    sha256Hex(compilerBefore) !== EXPECTED_CLANG_SHA256
  ) {
  ...
  const compilation = spawnSync(
    ...
    [
      ...
      "-Wextra",
      "-Werror",
      `-DV138_CONTROLLER_TOKEN_HEX=\"${token}\"`,
      capturedSourcePath,
      "-o",
      executable,
    ],
  ...
  if (
    sha256Hex(readFileSync(nativeSource)) !== sha256Hex(sourceBefore) ||
    sha256Hex(readFileSync("/usr/bin/clang")) !== sha256Hex(compilerBefore)
  ) {
    rmSync(directory, { recursive: true, force: true })
    fail("V138_SUCCESSOR_NATIVE_TOOLCHAIN_CHANGED")
  }
  ...
  if (
    ...
    executableStatus.uid !== process.getuid?.() ||
    (executableStatus.mode & 0o777) !== 0o700
  ) {
    rmSync(directory, { recursive: true, force: true })
  ...
  const capability =
    [
      ...
      String(rootStatus.dev),
      String(rootStatus.ino),
      sha256Hex(sourceBefore),
      sha256Hex(compilerBefore),
      sha256Hex(executableBefore),
    ].join("\t") + "\n"
  writeFileSync(capabilityPath, capability, { mode: 0o600, flag: "wx" })
  const capabilityDescriptor = openSync(
    capabilityPath,
      ...
      child = spawn(executable, [], {
        ...
        env:
          barrierTag === undefined || barrierTag === "force-spawn-failure"
            ? process.env
            : { ...process.env, V138_NATIVE_TEST_BARRIER: barrierTag },
      })
...
export const checkV138SuccessorControllerV3Source = (
  sourcePath: string,
): true => {
