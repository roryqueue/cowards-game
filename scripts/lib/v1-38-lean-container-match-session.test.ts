...
describe("lean Match-scoped hostile container session", () => {
  ...
  it("creates, starts, serves, and destroys exactly one controlled container for one Match", () => {
    ...
    const session = createLeanContainerMatchSession({
      matchId: "match:lean:a", containerName: "lean-a", ownershipLabel: "v1.38-lean-owner:lean-a", image: LEAN_CONTAINER_IMAGE, transport: fake.transport,
    })
  ...
  it("poisons and force-removes the session on malformed, surplus, or hostile output", () => {
    for (const stdout of ["not-json", '{"ok":true,"value":[],"extra":true}', '{"ok":true,"value":[]}\n{}']) {
      ...
      const session = createLeanContainerMatchSession({ matchId: "match:lean:b", containerName: "lean-b", ownershipLabel: "v1.38-lean-owner:lean-b", image: LEAN_CONTAINER_IMAGE, transport: fake.transport })
  ...
  it("poisons on timeout, signal, nonzero exit, stderr, or byte overflow", () => {
    ...
    for (const failure of failures) {
      ...
      const session = createLeanContainerMatchSession({ matchId: "match:lean:c", containerName: "lean-c", ownershipLabel: "v1.38-lean-owner:lean-c", image: LEAN_CONTAINER_IMAGE, transport: fake.transport })
      expect(() => session.adapter.execute({ source: "x", methodName: "soldierBrain", input: {}, outputByteLimit: 32 })).toThrow()
      expect(session.state).toBe("poisoned")
      expect(fake.calls.at(-2)?.[1][0]).toBe("rm")
  ...
  it("reports cleanup ambiguity, never reopens, and isolates Match identities", () => {
    ...
    const firstSession = createLeanContainerMatchSession({ matchId: "match:lean:one", containerName: "lean-one", ownershipLabel: "v1.38-lean-owner:lean-one", image: LEAN_CONTAINER_IMAGE, transport: first.transport })
    ...
    const secondSession = createLeanContainerMatchSession({ matchId: "match:lean:two", containerName: "lean-two", ownershipLabel: "v1.38-lean-owner:lean-two", image: LEAN_CONTAINER_IMAGE, transport: second.transport })
  ...
  it("fails creation closed and cleans up when start is ambiguous", () => {
    ...
    expect(() => createLeanContainerMatchSession({ matchId: "match:lean:d", containerName: "lean-d", ownershipLabel: "v1.38-lean-owner:lean-d", image: LEAN_CONTAINER_IMAGE, transport: fake.transport })).toThrow(/SESSION_START_FAILED/u)
  ...
  it.each([
    ...
    expect(() => createLeanContainerMatchSession({ matchId: "match:lean:e", containerName: "lean-e", ownershipLabel: "v1.38-lean-owner:lean-e", image: LEAN_CONTAINER_IMAGE, transport: fake.transport })).toThrow(/SESSION_CREATE_FAILED/u)
  ...
  it("refuses pre-existing and raced name collisions without reusing or deleting the occupant", () => {
    ...
    expect(() => createLeanContainerMatchSession({ matchId: "match:lean:f", containerName: "lean-f", ownershipLabel: "v1.38-lean-owner:lean-f", image: LEAN_CONTAINER_IMAGE, transport: occupied.transport })).toThrow(/NAME_COLLISION/u)
    expect(occupied.calls).toHaveLength(1)

    const raced = fakeTransport([absent(), result("", { status: 125 }), result("other-owner")])
    expect(() => createLeanContainerMatchSession({ matchId: "match:lean:g", containerName: "lean-g", ownershipLabel: "v1.38-lean-owner:lean-g", image: LEAN_CONTAINER_IMAGE, transport: raced.transport })).toThrow(/CLEANUP_INCOMPLETE/u)
  ...
  it("fails closed when removal or absence confirmation is ambiguous", () => {
    ...
    expect(() => createLeanContainerMatchSession({ matchId: "match:lean:h", containerName: "lean-h", ownershipLabel: "v1.38-lean-owner:lean-h", image: LEAN_CONTAINER_IMAGE, transport: cleanupFailed.transport })).toThrow(/CLEANUP_INCOMPLETE/u)

    const unknownAbsence = fakeTransport([result("", { status: null, error: new Error("inspect timeout") })])
    expect(() => createLeanContainerMatchSession({ matchId: "match:lean:i", containerName: "lean-i", ownershipLabel: "v1.38-lean-owner:lean-i", image: LEAN_CONTAINER_IMAGE, transport: unknownAbsence.transport })).toThrow(/NAME_CHECK_FAILED/u)
  })
})
