...
import { checkV138SuccessorControllerV3Source, V138_SUCCESSOR_CONTROLLER_V3_CLI, V138_SUCCESSOR_CONTROLLER_V3_OPERATIONS } from "./v1-38-bounded-retry-successor-controller-v3.js"

const run = (mode: string) => spawnSync(process.execPath, ["--import", "tsx", V138_SUCCESSOR_CONTROLLER_V3_CLI, mode], { cwd: process.cwd(), encoding: "utf8" })

describe("CR-01 controller-owned successor mutation closure", () => {
  it("exports only non-writing constants and source checks", async () => {
    const controller = await import("./v1-38-bounded-retry-successor-controller-v3.js")
    ...
    expect(Object.keys(controller).sort()).toEqual([
      "V138_SUCCESSOR_CONTROLLER_V3_CLI",
      "V138_SUCCESSOR_CONTROLLER_V3_OPERATIONS",
      "checkV138SuccessorControllerV3Source",
    ])
    ...
    expect(checkV138SuccessorControllerV3Source(V138_SUCCESSOR_CONTROLLER_V3_CLI)).toBe(true)
  })
  ...
  it("runs the complete write protocol only inside its controller-created temporary root", () => {
    ...
    expect(output.operations).toEqual(V138_SUCCESSOR_CONTROLLER_V3_OPERATIONS)
