...
const fixture = (model = false) => {
  ...
  const input = allocationFixture(), channel = model ? "model" as const : "automated" as const, reservation = { ...input.channels[0]!.perAttempt, matches: model ? 48 : 0, modelTokens: model ? 30 : 0, effortMilliseconds: 1000 }, sourceMessage = "Return explicit TypeScript source only."
  ...
  const job = { id: "one", channel, evaluationRole: "development_response" as const, operation: model ? "produce" as const : "unfilled" as const, producerRequestArtifactRoot, disclosureArtifactRoot, provenanceArtifactRoot, reviewArtifactRoot, participantId: "author", reviewerId: "reviewer", reservation, retryParentJobId: null }
  const allocation = createLeagueExecutionAllocation({ ...input, outputDirectories: { ...input.outputDirectories, responseFactory: repository.directory }, opportunities: { ...input.opportunities, attemptedCandidates: model ? 1 : 0, modelAttempts: model ? 1 : 0, modelTokens: reservation.modelTokens }, channels: input.channels.map((row) => row.channel === channel ? { ...row, disposition: "allocated", opportunities: 1, ceilings: reservation, perAttempt: reservation, participants: ["author"], reviewers: ["reviewer"] } : row), rounds: [{ ordinal: 0, acceptedSlots: 0, jobs: [job] }, input.rounds[1]!] })
