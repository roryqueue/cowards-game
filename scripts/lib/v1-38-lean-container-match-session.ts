...
const supervise=(q,worker,port,deadline)=>{let receipt;let receiptCount=0;let closeCount=0;let exitCount=0;let exitCode;let failure;let wake;const settled=new Promise(resolve=>{wake=resolve});const notify=()=>{if(failure!==undefined||receiptCount>1||closeCount>1||exitCount>1||(receiptCount===1&&closeCount===1&&exitCount===1))wake()};port.on("message",message=>{receiptCount+=1;if(!message||typeof message!=="object"||!exact(message,["requestId","kind","value"])||message.requestId!==q.requestId||message.kind!=="completion")failure=new Error("completion receipt invalid");else receipt=message.value;notify()});port.on("close",()=>{closeCount+=1;notify()});worker.on("error",()=>{failure=new Error("worker error");notify()});worker.on("exit",code=>{exitCount+=1;exitCode=code;notify()});return async()=>{let timer;try{const budget=remaining(deadline);if(budget<1)throw new Error("completion budget exhausted");await Promise.race([settled,new Promise((_,reject)=>{timer=setTimeout(()=>reject(new Error("completion lifecycle timeout")),budget)})]);if(failure!==undefined)throw failure;if(receiptCount!==1||closeCount!==1||exitCount!==1||exitCode!==0)throw new Error("completion lifecycle inconsistent");return receipt}finally{clearTimeout(timer)}}};
