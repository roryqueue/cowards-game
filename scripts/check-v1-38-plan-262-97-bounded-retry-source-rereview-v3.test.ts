...
describe("Plan 262-97 fresh corrected-source re-review", () => {
  ...
  it("derive-no-publish is deterministic and changes no review or downstream destination", () => {
    ...
    expect(before.find((item) => item.path === V138_PLAN_262_97_REVIEW_PATH)?.type).toMatch(/^(absent|regular)$/u)
    expect(before.find((item) => item.path === V138_PLAN_262_97_REPORT_PATH)?.type).toMatch(/^(absent|regular)$/u)
