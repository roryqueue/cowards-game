...
export const createFactoryExecutionEvidenceFixture = async () => {
  ...
  const searchValue = { schemaVersion: "factory-teacher-search-evidence-v1", charged: true, runs: 1, allocationOrdinal: 0, request: { maxDepth: 3, maxNodes: 128 }, receipt }, teacherSearchArtifactRoot = publishRooted(repository, "factory-teacher-search-evidence-v1", searchValue)
  const trainingValue = { schemaVersion: "factory-teacher-training-evidence-v1", searchArtifactRoot: teacherSearchArtifactRoot, records: [legal], student }, teacherTrainingArtifactRoot = publishRooted(repository, "factory-teacher-training-evidence-v1", trainingValue)
  const audit = deriveFactorySharedHelperAudit(ingestions), sharedHelperAuditArtifactRoot = retain(audit)
  const witnesses = { S01: deriveFactoryNegativeWitness(ingestions.S01), S03: deriveFactoryNegativeWitness(ingestions.S03), S05: deriveFactoryNegativeWitness(ingestions.S05) }, negativeWitnessArtifactRoots = { S01: retain(witnesses.S01), S03: retain(witnesses.S03), S05: retain(witnesses.S05) }
  ...
  return { repository, fresh, executionEvidenceArtifactRoot, refs, roots: { sourceReviewArtifactRoot, teacherSearchArtifactRoot, teacherTrainingArtifactRoot, sharedHelperAuditArtifactRoot, ...negativeWitnessArtifactRoots }, values: { executionValue, reviewValue, searchValue, trainingValue, audit, witnesses } }
}
