...
export const V138_LIVE_V12_PATHS = Object.freeze({
  ...
  plan120Payload: ".planning/artifacts/v1.38-plan-262-120-live-v12-custody-review-payload-v2.json",
  plan120Review: `${PHASE}/262-120-REVIEW-v2.md`,
  plan120Carrier: ".planning/artifacts/v1.38-plan-262-120-live-v12-custody-review-carrier-v2.json",
})
...
const PLAN_114_PATHS = Object.freeze([
  Object.freeze([V138_LIVE_V12_PATHS.plan114PayloadV1, V138_LIVE_V12_PATHS.plan114ReviewV1, V138_LIVE_V12_PATHS.plan114CarrierV1] as const),
  Object.freeze([V138_LIVE_V12_PATHS.plan114PayloadV2, V138_LIVE_V12_PATHS.plan114ReviewV2, V138_LIVE_V12_PATHS.plan114CarrierV2] as const),
] as const)
const PLAN_120_PATHS = Object.freeze([
...
const PLAN_118_V1_PATHS = Object.freeze([
  V138_LIVE_V12_PATHS.plan118PayloadV1,
  V138_LIVE_V12_PATHS.plan118ReviewV1,
  V138_LIVE_V12_PATHS.plan118CarrierV1,
] as const)
...
export const V138_LIVE_V12_FORBIDDEN_PRE_EFFECT = Object.freeze([
  ...PLAN_120_PATHS,
  ...PRODUCER_OUTPUTS,
  ...DOWNSTREAM_OUTPUTS,
])
export const V138_LIVE_V12_MODES = Object.freeze([
  "--check-source-only",
...
export const V138_LIVE_V12_REVIEWED_SOURCE_PATHS = Object.freeze([
  ...
  V138_LIVE_V12_PATHS.source,
  V138_LIVE_V12_PATHS.tests,
] as const)
...
const readRegularNoFollow = (
  ...
  try {
    ...
    if (!before.isFile() || before.size > maximumBytes || (before.mode & 0o7777) !== expectedMode)
      fail(`V138_LIVE_V12_CURRENT_ENTRY_INVALID:${repoPath}`)
    const bytes = readFileSync(descriptor)
    const after = fstatSync(descriptor)
    if (before.dev !== after.dev || before.ino !== after.ino || before.size !== after.size ||
        before.mtimeMs !== after.mtimeMs || before.ctimeMs !== after.ctimeMs)
      fail(`V138_LIVE_V12_CURRENT_ENTRY_CHANGED:${repoPath}`)
    return bytes
  } catch (error) {
    if (error instanceof Error && error.message.startsWith("V138_LIVE_V12_")) throw error
    fail(`V138_LIVE_V12_CURRENT_ENTRY_INVALID:${repoPath}`)
...
const exactPublication = (
  ...
  if (canonical(changed) !== canonical(expected)) fail("V138_LIVE_V12_PUBLICATION_SCOPE_INVALID")
  const bytes = paths.map((repoPath) => {
    const entry = git(root, ["ls-tree", commit, "--", repoPath])
    const match = /^100644 blob ([0-9a-f]{40})\t(.+)$/u.exec(entry)
    if (match === null || match[2] !== repoPath) fail(`V138_LIVE_V12_PUBLICATION_MODE_INVALID:${repoPath}`)
    const committed = gitBytes(root, commit, repoPath)
    if (!readRegularNoFollow(root, repoPath).equals(committed))
      fail(`V138_LIVE_V12_PUBLICATION_CURRENT_BYTES_INVALID:${repoPath}`)
    return committed
  })
  noRewrite(root, commit, paths)
  return Object.freeze({
    payload: jsonBytes(bytes[0]!, "V138_LIVE_V12_PAYLOAD_INVALID"),
    reviewBytes: bytes[1]!,
    carrier: jsonBytes(bytes[2]!, "V138_LIVE_V12_CARRIER_INVALID"),
  })
}
const assertAbsent = (root: string, paths: readonly string[]): void => {
  for (const repoPath of paths) if (pathPresent(root, repoPath))
    fail(`V138_LIVE_V12_FORBIDDEN_DESTINATION_PRESENT:${repoPath}`)
}
...
const authenticateAllowedLiveV11Successor = (root: string) => {
  ...
  if (plan118.payload.schemaVersion !== "v1.38-plan-262-118-live-v11-custody-review-payload-v1" ||
    ...
    fail("V138_LIVE_V12_PLAN118_V1_HISTORY_INVALID")
  ...
  if (reviewedEntries.some((valid) => !valid)) fail("V138_LIVE_V12_PLAN117_V1_HISTORY_INVALID")
  ...
  if (git(root, ["rev-list", "--parents", "-n", "1", ALLOWED_CORRECTION_COMMIT]) !==
    ...
    fail("V138_LIVE_V12_CORRECTION_PARENT_INVALID")
  if (git(root, ["diff-tree", "--no-commit-id", "--name-status", "-r", ALLOWED_CORRECTION_COMMIT]) !==
      `M\t${LIVE_V11_PATHS[1]}`)
    fail("V138_LIVE_V12_CORRECTION_SCOPE_INVALID")
  ...
  if (correctionEntries.some((valid) => !valid)) fail("V138_LIVE_V12_CORRECTION_ENTRY_INVALID")
  const parentEntries = [LIVE_V11_SOURCE_BLOB, LIVE_V11_REVIEWED_TEST_BLOB].map((blob, index) =>
    git(root, ["ls-tree", ALLOWED_CORRECTION_PARENT, "--", LIVE_V11_PATHS[index]!]) ===
      `100644 blob ${blob}\t${LIVE_V11_PATHS[index]}`)
  if (parentEntries.some((valid) => !valid)) fail("V138_LIVE_V12_CORRECTION_PARENT_ENTRY_INVALID")
  for (const [index, repoPath] of LIVE_V11_PATHS.entries()) {
    ...
    if (!readRegularNoFollow(root, repoPath).equals(gitBytes(root, ALLOWED_CORRECTION_COMMIT, repoPath)) ||
      ...
      fail(`V138_LIVE_V12_CORRECTION_CURRENT_ENTRY_INVALID:${repoPath}`)
  }
  ...
  return Object.freeze({
    ...history,
    allowedHistoryRoot: rooted("v138-plan-262-119-live-v12-allowed-history-v1", history),
  })
}
...
const authenticatePublicationHistory = (root: string): void => {
  ...
  for (const [index, commit] of PLAN_116_COMMITS.entries()) {
    ...
    if (publication.payload.payloadRoot !== roots.payload || publication.carrier.reviewRoot !== roots.review ||
        ...
        publication.payload.liveInvoked !== false || publication.payload.freshCharged !== 0 ||
        publication.payload.freshAccepted !== 0 || publication.payload.downstreamAuthority !== "denied")
      fail(`V138_LIVE_V12_PLAN116_V${index + 1}_INVALID`)
    if (index > 0 && publication.payload.supersedesPublicationCommit !== PLAN_116_COMMITS[index - 1])
      fail(`V138_LIVE_V12_PLAN116_V${index + 1}_SUPERSESSION_INVALID`)
  }
}
const authenticateSupplement = (root: string): Json => {
  ...
  if (scope !== `A\t${V138_LIVE_V12_PATHS.supplementV3}`) fail("V138_LIVE_V12_SUPPLEMENT_SCOPE_INVALID")
  const entry = git(root, ["ls-tree", SUPPLEMENT_COMMIT, "--", V138_LIVE_V12_PATHS.supplementV3])
  if (entry !== `100644 blob ${SUPPLEMENT_BLOB}\t${V138_LIVE_V12_PATHS.supplementV3}`)
    fail("V138_LIVE_V12_SUPPLEMENT_ENTRY_INVALID")
  const bytes = gitBytes(root, SUPPLEMENT_COMMIT, V138_LIVE_V12_PATHS.supplementV3)
  if (sha(bytes) !== SUPPLEMENT_SHA256 || !readRegularNoFollow(root, V138_LIVE_V12_PATHS.supplementV3).equals(bytes))
    fail("V138_LIVE_V12_SUPPLEMENT_BYTES_INVALID")
  noRewrite(root, SUPPLEMENT_COMMIT, [V138_LIVE_V12_PATHS.supplementV3])
  const supplement = jsonBytes(bytes, "V138_LIVE_V12_SUPPLEMENT_INVALID")
  if (supplement.schemaVersion !== "v1.38-successor-source-seal-v13-executable-custody-supplement-v3" ||
      ...
      supplement.downstreamAuthority !== "denied") fail("V138_LIVE_V12_SUPPLEMENT_SEMANTICS_INVALID")
  return supplement
}
const authenticatePairAndStop = (root: string): Readonly<{ seal: Json; envelope: Json }> => {
  requireAncestor(root, PAIR_COMMIT)
  const seal = jsonBytes(gitBytes(root, PAIR_COMMIT, V138_LIVE_V12_PATHS.seal), "V138_LIVE_V12_SEAL_INVALID")
  const envelope = jsonBytes(gitBytes(root, PAIR_COMMIT, V138_LIVE_V12_PATHS.envelope), "V138_LIVE_V12_ENVELOPE_INVALID")
  if (!readRegularNoFollow(root, V138_LIVE_V12_PATHS.seal, 8 * 1024 * 1024, 0o600)
      .equals(gitBytes(root, PAIR_COMMIT, V138_LIVE_V12_PATHS.seal)) ||
      !readRegularNoFollow(root, V138_LIVE_V12_PATHS.envelope, 8 * 1024 * 1024, 0o600)
        .equals(gitBytes(root, PAIR_COMMIT, V138_LIVE_V12_PATHS.envelope)))
    fail("V138_LIVE_V12_PAIR_CURRENT_BYTES_INVALID")
  noRewrite(root, PAIR_COMMIT, [V138_LIVE_V12_PATHS.seal, V138_LIVE_V12_PATHS.envelope])
  requireAncestor(root, PLAN_93_STOP_COMMIT)
  if (!readRegularNoFollow(root, V138_LIVE_V12_PATHS.plan93).equals(gitBytes(root, PLAN_93_STOP_COMMIT, V138_LIVE_V12_PATHS.plan93)))
    fail("V138_LIVE_V12_PLAN93_CURRENT_BYTES_INVALID")
  noRewrite(root, PLAN_93_STOP_COMMIT, [V138_LIVE_V12_PATHS.plan93])
  if (seal.sealRoot !== SEAL_ROOT || seal.protectedHistoryRoot !== PROTECTED_HISTORY_ROOT ||
    ...
    fail("V138_LIVE_V12_PAIR_SEMANTICS_INVALID")
  return Object.freeze({ seal, envelope })
}

export type V138LiveV12SourceAdmission = Readonly<{
...
const authenticateV138LiveV12InvariantCustody = (rootInput: string): V138LiveV12SourceAdmission => {
  ...
  assertAbsent(root, [V138_LIVE_V12_PATHS.supplementV1, V138_LIVE_V12_PATHS.supplementV2])
  return Object.freeze({
...
export const authenticateV138LiveV12SourceOnly = (rootInput: string): V138LiveV12SourceAdmission => {
  const source = authenticateV138LiveV12InvariantCustody(rootInput)
  assertAbsent(path.resolve(rootInput), [...PRODUCER_OUTPUTS, ...DOWNSTREAM_OUTPUTS])
  return source
}

const plan120PayloadRoot = (body: Json): Sha => rooted("v138-plan-262-120-live-v12-custody-review-payload-v2", body)
const plan120ReviewRoot = (body: Json): Sha => rooted("v138-plan-262-120-live-v12-custody-review-markdown-v2", body)
const plan120CarrierRoot = (body: Json): Sha => rooted("v138-plan-262-120-live-v12-custody-review-carrier-v2", body)

const renderPlan120Contracts = (input: {
  ...
  const payload = Object.freeze({ ...body, payloadRoot: plan120PayloadRoot(body) })
  const reviewBody = {
  ...
  const reviewRoot = plan120ReviewRoot(reviewBody)
  ...
  const carrier = Object.freeze({ ...carrierBody, carrierRoot: plan120CarrierRoot(carrierBody) })
  return Object.freeze({ payload, reviewBytes, reviewRoot, carrier, plan120PublicationCommit: input.plan120PublicationCommit })
}

export const deriveV138LiveV12ProspectiveContractsForReview = (input: {
  repoRoot: string
  reviewedSourceCommit: string
  plan120PublicationCommit: string
  reviewedLocalExecutionClosureRoot?: Sha
}) => {
  const source = authenticateV138LiveV12SourceOnly(input.repoRoot)
  ...
  const contracts = renderPlan120Contracts({
    ...
    reviewedLocalExecutionClosureRoot:
      input.reviewedLocalExecutionClosureRoot ?? reviewedClosure.localExecutionClosureRoot,
    plan120PublicationCommit: input.plan120PublicationCommit,
  })
...
export const checkV138LiveV12ProspectiveCustodyForReview = (input: {
  ...
  if (canonical(input.plan120.payload) !== canonical(exact.payload) ||
      ...
      input.plan120.reviewRoot !== exact.reviewRoot)
...
const authenticateFutureCustody = (rootInput: string, boundary: "pre" | "post" = "pre") => {
  const root = path.resolve(rootInput)
  const publicationCommit = git(root, ["log", "--diff-filter=A", "--format=%H", "--", V138_LIVE_V12_PATHS.plan120Payload])
  if (!/^[0-9a-f]{40}$/u.test(publicationCommit)) fail("V138_LIVE_V12_PLAN120_PUBLICATION_INVALID")
  const plan120 = exactPublication(root, publicationCommit, PLAN_120_PATHS)
  const subjectCommit = plan120.payload.subjectCommit
  if (typeof subjectCommit !== "string" || !/^[0-9a-f]{40}$/u.test(subjectCommit))
    fail("V138_LIVE_V12_PLAN120_SUBJECT_INVALID")
  const source = authenticateV138LiveV12InvariantCustody(root)
  ...
  const checked = checkV138LiveV12ProspectiveCustodyForReview({
    ...
    plan120PublicationCommit: publicationCommit,
    plan120: { ...plan120, reviewRoot: plan120.carrier.reviewRoot },
  })
...
export const authenticateV138LiveV12FutureCustodyForReview = authenticateFutureCustody

export const checkV138LiveV12PostRunOutputCustodyForReview =
  checkV138LiveV10PostRunOutputCustodyForReview
export const computeV138LiveV12ReproductionV17ReceiptRoot =
  computeV138LiveV10ReproductionV17ReceiptRoot
export const checkV138LiveV12ReproductionV17ForReview =
  checkV138LiveV10ReproductionV17ForReview

export const inspectV138LiveV12ProductionBoundarySourceForReview = (source: string) => {
  const sourceFile = ts.createSourceFile(V138_LIVE_V12_PATHS.source, source, ts.ScriptTarget.Latest, true, ts.ScriptKind.TS)
  ...
  if (!exactImport || importedBindings.length !== 2 || producerReferences !== 2 ||
    ...
    fail("V138_LIVE_V12_PRODUCTION_BOUNDARY_INVALID")
  return Object.freeze({
...
export const inspectV138LiveV12ProductionBoundaryForReview = (rootInput: string) =>
  inspectV138LiveV12ProductionBoundarySourceForReview(
    readRegularNoFollow(path.resolve(rootInput), V138_LIVE_V12_PATHS.source).toString("utf8"),
  )

export const runV138ReviewedBoundedLiveEnvelopeV12 = async (repoRoot: string): Promise<void> => {
  ...
  try {
    ...
    try {
      ...
      if (after.canonicalLocalExecutionClosureRoot !== ready.canonicalLocalExecutionClosureRoot)
        fail("V138_LIVE_V12_CANONICAL_LOCAL_CLOSURE_CHANGED")
...
const resolveCurrentSubjectCommit = (root: string): string => {
  const commit = git(root, ["log", "-1", "--format=%H", "--", V138_LIVE_V12_PATHS.source, V138_LIVE_V12_PATHS.tests])
  if (!/^[0-9a-f]{40}$/u.test(commit)) fail("V138_LIVE_V12_SUBJECT_COMMIT_INVALID")
  return commit
}

export const executeV138LiveV12Cli = async (
  args: readonly string[],
  injected?: Partial<{ repoRoot: string; writeOutput: (value: string) => void }>,
): Promise<void> => {
  if (args.length !== 1 || !V138_LIVE_V12_MODES.includes(args[0] as never))
    fail("V138_LIVE_V12_ARGUMENTS_INVALID")
  ...
  if ((args[0] === "--check-prospective-custody" || args[0] === "--check-post-run-custody") &&
      !pathPresent(root, V138_LIVE_V12_PATHS.plan120Payload)) {
    const subjectCommit = resolveCurrentSubjectCommit(root)
    const prospective = deriveV138LiveV12ProspectiveContractsForReview({
      repoRoot: root,
...
if (isEntrypoint) await executeV138LiveV12Cli(process.argv.slice(2))
