...
const createV117ServiceArtifacts = () => {
  ...
  const claims: RuntimeSemanticReceiptClaimsV117 = {
    ...
    ledgerPoststateRoot: identity("a"),
    chronicleCanonicalHash: identity("b"),
    finalStateCanonicalHash: identity("c"),
    reconstructedTerminalStateHash: identity("d"),
    outcomeCanonicalHash: identity("e"),
  ...
  const response: RuntimeExecutionServiceSuccessResponseV117 = {
    ...
    result: {
      privacy: "internal_runtime_result",
      chronicle: { events: [], snapshots: [] },
      finalState: { matchId: request.matchId, status: "complete" },
      outcome: { kind: "draw", reason: "fixture" },
      ledgerPoststateRoot: claims.ledgerPoststateRoot,
      runtimeViolationEventCount: 0,
      semanticReceipt: {
        ...claims,
        signature: `hmac-sha256:${createHmac(
          "sha256",
          "fixture-only:runtime-service-v1.17:secret",
        )
