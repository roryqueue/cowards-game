...
export const checkV137ReleaseTag = (options: { repoRoot?: string; archiveCommit?: string; mode?: "pretag-archive" | "post-tag" } = {}): V137ReleaseTagResult => {
...
const direct = () => { const args = process.argv.slice(2); const pre = args[0] === "--pretag-archive" && args.length === 2 ? args[1] : undefined; const valid = (pre && args.length === 2) || (!pre && args.length === 0); if (!valid) throw new TypeError("V137_RELEASE_TAG_MODE_INVALID"); const result = checkV137ReleaseTag(pre ? { mode: "pretag-archive", archiveCommit: pre } : {}); if (result.findings.length) throw new TypeError(result.findings.map((finding) => finding.code).join(",")); process.stdout.write(`${JSON.stringify({ mode: result.mode, proof08: result.proof08 })}\n`) }
