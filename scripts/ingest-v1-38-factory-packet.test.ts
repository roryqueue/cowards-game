...
describe("named private factory ingestion", () => {
  ...
  it("re-admits a v2 bundle only with its exact private unavailable-snapshot provenance", async () => {
    ...
    const rawResponseRecordValue = { format: "codex-exec-json" as const, bodyUtf8: JSON.stringify({ source: modelSource, usage: { inputTokens: 1, outputTokens: 1, cachedInputTokens: 0 } }) }
