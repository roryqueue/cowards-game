...
import {
  assertV138Plan120PublishedLocalClosureForReview,
  executeV138Plan120DisposableModes,
  inspectV138Plan120BoundarySourceForReview,
  renderV138Plan120EvidenceForReview,
} from "./check-v1-38-plan-262-120-live-v12-custody-v2.js"

const ROOT = path.resolve(path.dirname(new URL(import.meta.url).pathname), "..")

describe("Plan 262-120 independent live-v12 custody review v2", () => {
  it("rejects producer aliases, relocated calls, and injectable effect seams", () => {
    const source = readFileSync(path.join(ROOT,
      "scripts/run-v1-38-bounded-retry-envelope-v3-live-v12.ts"), "utf8")
    expect(inspectV138Plan120BoundarySourceForReview(source)).toMatchObject({
      producerCallSites: 1,
      producerCalls: 0,
      readinessInvoked: false,
      liveInvoked: false,
    })
    expect(() => inspectV138Plan120BoundarySourceForReview(
      source.replace("await runV138V3ProductionLive", "const alias = runV138V3ProductionLive; await alias"),
    )).toThrow("V138_PLAN120_PRODUCTION_BOUNDARY_INVALID")
    expect(() => inspectV138Plan120BoundarySourceForReview(
    ...
    for (const moved of movedCalls)
      expect(() => inspectV138Plan120BoundarySourceForReview(moved)).toThrow(
        "V138_PLAN120_PRODUCTION_BOUNDARY_INVALID",
      )
  })
  ...
  it("executes exactly six producer-incapable observations and renders literal-zero eligibility", () => {
    ...
    expect(assertV138Plan120PublishedLocalClosureForReview({
  ...
  it("blocks eligibility when a rooted subject finding exists", () => {
    const evidence = renderV138Plan120EvidenceForReview(ROOT, [{
      ...
      severity: "critical",
      subject: "scripts/run-v1-38-bounded-retry-envelope-v3-live-v12.ts",
