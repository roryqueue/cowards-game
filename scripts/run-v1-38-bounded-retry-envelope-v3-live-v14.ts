...
import { chmodSync, closeSync, constants, fstatSync, lstatSync, mkdirSync, mkdtempSync, openSync,
  readFileSync, readdirSync, realpathSync, rmSync, writeFileSync } from "node:fs"
...
import { encodeV138RetryV3CanonicalJson, checkV138InactiveRetryV3Envelope, deriveV138RetryV3State } from "./lib/v1-38-bounded-retry-envelope-v3.js"
...
export const validateV138LiveV14PublishedContractForReview = (input: unknown): true => {
  ...
  if (historical.sourceCommit !== FINAL142 || historical.summaryCommit !== SUMMARY142 || historical.trackingCommit !== TRACK142 ||
    ...
    !isSha(historical.observationsRoot)) fail("HISTORICAL142_INVALID")
...
const committed = (h: History, commit: string, paths: readonly string[], noRewrite = true): Record<string, Buffer> => {
  ...
  const batch = readV138WorkspaceBatch(h.root, paths)
...
const fixedHistory = (h: History) => {
  ...
  scope(h, AMENDMENT, [["M", ".planning/ROADMAP.md"], ["M", ".planning/STATE.md"], ["M", `${PHASE}/262-110-PLAN.md`],
    ["A", `${PHASE}/262-120-SUMMARY.md`], ["M", `${PHASE}/262-122-PLAN.md`], ["M", `${PHASE}/262-93-PLAN.md`], ["A", `${PHASE}/262-93-SUMMARY.md`]])
  const pins = [
  ...
  for (const [p, blob] of [[`${PHASE}/262-93-PLAN.md`, "d540a5a7b0f7200ed86287a3744e46ebd66987bd"],
...
const repositoryClosure = (root: string, roots: readonly string[]) => {
  ...
  const seen = new Set<string>(); const edges = new Map<string, {from: string; to: string}>(); const entries: FileEntry[] = []
  ...
  while (pending.length) {
    const p = pending.shift()!; if (seen.has(p)) continue
    if (!relative(p)) fail("DEPENDENCY_PATH")
    seen.add(p); const absolute = path.join(root, p)
    const bytes = regular(absolute)
    entries.push({ path: p, mode: lstatSync(absolute).mode & 0o111 ? "100755" : "100644",
      blob: createHash("sha1").update(`blob ${bytes.length}\0`).update(bytes).digest("hex"), sha256: sha(bytes) })
    const add = (to: string) => {
      if (!relative(to)) fail("DEPENDENCY_PATH")
    ...
    if (/\.[cm]?[jt]sx?$/.test(p)) {
      ...
      const visit = (node: ts.Node) => {
        ...
        if (spec && !spec.startsWith("node:") && !["fs", "path", "crypto", "os", "url", "util", "events", "stream", "buffer", "child_process"].includes(spec)) {
          const resolved = ts.resolveModuleName(spec, absolute, options, ts.sys).resolvedModule
          if (!resolved) fail("DEPENDENCY_UNRESOLVED")
          const filename = resolved.resolvedFileName
          if (!filename.includes("/node_modules/")) add(path.relative(root, filename).split(path.sep).join("/"))
        }
        if (ts.isStringLiteral(node) && /^scripts\/native\/[A-Za-z0-9_.-]+\.c$/.test(node.text)) add(node.text)
  ...
  const batch = readV138WorkspaceBatch(root, entries.map(e => e.path))
